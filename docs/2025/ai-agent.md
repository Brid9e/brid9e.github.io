...
一开始想解决的是：**不只要多路 MCP，还要像一个真正的 Agent 一样——同一轮对话里可以反复「想一步 → 调工具 → 看结果 → 再想」**，直到任务结束或触达上限。MCP（Model Context Protocol）把工具拆成独立进程或服务；我在上面叠的是 **HTTP 网关 + 多连接聚合 + 模型侧多轮工具循环**，再用 Vite 页面把流式过程摊开给人看。
